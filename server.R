...
server <- function(input, output, session) {
  ...
  getData <- reactive({
    if(is.null(input$networkTXTfile)) return(NULL)
    else return(TRUE)
  })
  ...
  observeEvent(input$exampleButton, {
    ...
    randomNet <- TRUE
    network <- BoolNet::generateRandomNKNetwork(15,3)
    ...
    allTable <- as.data.frame(t(measuresmatrix))
  ...
  observe({input$networkTXTfile #Alternative, works as well, but does not react to changing input$threshold
