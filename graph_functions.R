...
rm(list = ls()) #Clear global environment before launching the app
...
conv2adjmat <- function(network, inputcorrected = FALSE){
  "Converts a network object to adjacency matrix. 
...
meanofminattrdists <- function(network, modification, modgene, method = "trinary"){
  ...
  modnetwork <- network
  if (modification == "overexpression"){modnetwork$fixed[modgene] <- 1}
  else if (modification == "knockout"){modnetwork$fixed[modgene] <- 0}
  modattrs <- getAttractors(modnetwork, method = "sat.exhaustive")
...
meanofminattrdistsbygene <- function(network, modification, method = "trinary"){
  ...
  if (modification %in% c("overexpression", "knockout") == F){return("Not a valid modification, choose overexpression or knockout.")}
  res <- rep(NA, length(network$genes))
  for (g in 1:length(network$genes)){
    res[g] <- meanofminattrdists(network, modification, g, method)
  }
  return(res)
}
meanofminattrdistsmerged <- function(network, method = "trinary"){
  "Returns a vector choosing the worst-case output from either OE or KO for any gene, 
  i.e. the most impactful perturbation."
  res <- OE <- KO <- rep(NA, length(network$genes))
  for (g in 1:length(network$genes)){
    OE[g] <- meanofminattrdists(network, modification="overexpression", g, method)
    KO[g] <- meanofminattrdists(network, modification="knockout", g, method)
    res[g] <- max(OE[g], KO[g])
  }
...
attrcoverageaftermod <- function(network, modification, modgene, method = "trinary"){
  if (method %in% c("trinary", "prob") == F){return("Not a valid method, choose trinary or prob.")}
  if (modification %in% c("overexpression", "knockout") == F){return("Not a valid modification, choose overexpression or knockout.")}
  attrs <- getAttractors(network, method = "sat.exhaustive")
  modnetwork <- network
  ...
  losscounter <- 0 #count phenotypes that have no match in modnetwork, finally divide by nr. of orig.attrs
  for (a in 1:length(attrs$attractors)){
...
attrcoverageaftermodbygene <- function(network, modification, method = "trinary"){
  ...
  res <- rep(NA, length(network$genes))
  for (g in 1:length(network$genes)){
    res[g] <- attrcoverageaftermod(network, modification, g, method)
  }
  names(res) <- network$genes
  return(res)
}
attrcoveragemerged <- function(network, merging="min", method="trinary"){
  ...
  res <- OE <- KO <- rep(NA, length(network$genes))
  for (g in 1:length(network$genes)){
    OE[g] <- attrcoverageaftermod(network, modification="overexpression", g, method=method)
    KO[g] <- attrcoverageaftermod(network, modification="knockout", g, method=method)
...
newattrgain <- function(network, modification, modgene, method="trinary"){
  if (method %in% c("trinary", "prob") == F){return("Not a valid method, choose trinary or prob.")}
  ...
  adjmat <- conv2adjmat(network, inputcorrected = F)
  size <- dim(adjmat)[1]
  attrs <- getAttractors(network, method = "sat.exhaustive")
  nrattrs <- length(attrs$attractors)
  modnetwork <- network
  if (modification=="overexpression"){modnetwork$fixed[modgene] <- 1} else if (modification=="knockout"){modnetwork$fixed[modgene] <- 0}
  modattrs <- getAttractors(modnetwork, method = "sat.exhaustive")
...
newattrgainbygene <- function(network, modification, method="trinary"){
  adjmat <- conv2adjmat(network, inputcorrected = F)
  size <- dim(adjmat)[1]
  res <- rep(NA, size)
  for (g in 1:size){
    res[g] <- newattrgain(network=network, modification=modification, modgene = g, method=method)
  }
  names(res) <- network$genes
  return(res)
}
newattrgainbygenemerged <- function(network, method="trinary"){
  adjmat <- conv2adjmat(network, inputcorrected = F)
  size <- dim(adjmat)[1]
  res <- rep(NA, size)
  OEimpact <- newattrgainbygene(network, modification="overexpression", method=method)
  KOimpact <- newattrgainbygene(network, modification="knockout", method=method)
  for (g in 1:size){
...
suppBoolFunc <- function(network, bfunc){
  ...
  inputs <- network$interactions[[bfunc]]$input
  ...
  #stringFunc <- network$interactions[[bfunc]]$expression
  ...
  for (i in 1:dim(allinputcombis)[1]){
    ...
    newstate <- unlist(stateTransition(network, state = initialvec, type="synchronous"))
...
DP_j <- function(network, j){
  ...
  res <- 0
  for (i in inputs){
    MI_fi_Xj <- MutInfo(network, fi = i, j = j)
    res <- res +  MI_fi_Xj
  }
  return(res)
}
DeterminativePower <- function(network){
  size <- length(network$genes)
  DPvec <- rep(NA, size)
  for (j in 1:size){
    DPvec[j] <- DP_j(network, j)
  }
  names(DPvec) <- network$genes
  return(DPvec)
}

nroutlinks <- function(network){
  size <- length(network$genes)
...
calculateMeasures <- function(network){
  adjmat <- conv2adjmat(network)
  gr <- igraph::graph_from_adjacency_matrix(adjmat)
  #output$calculating <- renderText({"Calculating static measures on interaction graph..."})
  DP <- DeterminativePower(network)
  VB <- VertexBetweenness(adjmat)
  Z <- singlemoduleZ(network)
  measures <- list(VB, DP, Z)
  names(measures) <- c("VB", "DP", "Z")
  return(measures)
}

classify <- function(network,VB,DP,Z,threshold){
  adjmat <- conv2adjmat(network)
  DPvalues <- round(DP,3)
  VBvalues <- round(VB,3)
  Zvalues <- round(Z,3)
  #output$calculating <- renderText({"Calculation of static measures done!"})
  Zpercentiles <- Zvalues #map Zvalues to positive range to be used as vertex sizes
  for (g in 1:length(Zvalues)){
    Zpercentiles[g] <- percentile(-Zvalues, -Zvalues[g])
  }
  #measuresMatrix <- matrix(NA, nrow=3, ncol=length(network$genes))
  #measuresMatrix[1,] <- VBvalues
  #measuresMatrix[2,] <- DPvalues
  #measuresMatrix[3,] <- Zvalues
  #measuresdf <- as.data.frame(measuresMatrix)
  #colnames(measuresdf) <- network$genes
  #rownames(measuresdf) <- c("Vertex Betweenness", "Determinative Power", "Connectivity (Z-score)")
  
  #SortedVB <- as.data.frame(t(sort(VBvalues, decreasing = TRUE)))
  #rownames(SortedVB) <- "VB"
  #SortedDP <- as.data.frame(t(sort(DPvalues, decreasing = TRUE)))
  #rownames(SortedDP) <- "DP"
  #SortedZ <- as.data.frame(t(sort(Zvalues, decreasing = TRUE)))
  #rownames(SortedZ) <- "Z"
  ...
  recommendation <- which(StatZQuantRatio > 0)
  
  #define sets
  NSindices <- which(colnames(adjmat) %in% setdiff(network$genes, names(VBnDP)))
  hubindices <- which(Z > 2.5) #Also non-selected hubs highlighted in blue, was previously = setdiff(which(Z > 2.5), NSindices)
  nonhubindices <- setdiff(seq(1:length(network$genes)), hubindices)
  PMindices <- which(colnames(adjmat) %in% setdiff(names(recommendation), colnames(adjmat)[c(NSindices)]))
  NMindices <- setdiff(seq(1:length(network$genes)), c(NSindices, PMindices))
  
  targets <- colnames(adjmat)
  names(targets)[hubindices] <- "Hub"
  names(targets)[nonhubindices] <- "Non-Hub"
  targets <- targets[PMindices]
  decreasingMismatchOrder <- names(sort(StatZQuantRatio[recommendation], decreasing = TRUE))
  targets <- targets[match(decreasingMismatchOrder, targets)]
  
  GKswithMismatch <- as.data.frame(t(sort(StatZQuantRatio[recommendation], decreasing = TRUE)))
  
  resultlist <- list(PMindices, NMindices, NSindices, hubindices, GKswithMismatch)
  names(resultlist) <- c("PMindices", "NMindices", "NSindices", "hubindices", "GKswithMismatch")
  return(resultlist)
}
